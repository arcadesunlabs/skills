...
| Flat domain (e.g. `metrics/`, `auth/`)    | `docs/<domain>/handoff.md`                                                                                      |
...
**Naming:** use `handoff.md` (not numbered `01`–`04` — those are spec/tech/plan/tasks). Replace when superseded during active work. **After delivery,** [`write-finalize-docs`](../write-finalize-docs/SKILL.md) merges useful facts into `02-tech.md` and deletes `handoff.md`.

Folder names mirror `web/src/components/<domain>/` in kebab-case (`task_editor` → `task-editor`). See [docs/index.md](../../../docs/index.md) for the index and conventions.

## What to include

- Current objective and Trello card (`PM-XXX`) if any
